#!/usr/bin/env python3
"""Isolated F044-D35 still-later-following-continuation-run probe over exact GREEN D34.

No repair is performed. The exact D34 entrypoint is retained byte-for-byte as
`scripts/verify_repository_probe_base.py`; this wrapper adds exactly one
representative where the third following sibling owns two ordinary continuation
...
PRIOR_GREEN_D34_BLOB_SHA = "e338a69b385de05c17901bd004e27bab4c203172"

core = prior.core
_prior_synthetic_check = core.check_synthetic_rejections_and_transition_positives


def _check_f044d35_probe() -> None:
    source = (
        "- Parent:\n"
    ...
    try:
        ...
        if "publishes forbidden self-promotion" not in str(exc):
            raise
        print(
            "[PASS] F044-D35 probe reproduces still-later-following-continuation-run false positive"
        )
        return
    raise core.VerificationError(
        "F044-D35 probe NOT REPRODUCED: still-later-following-continuation-run representative is already accepted"
    )


def _synthetic_check_with_probe() -> None:
    _prior_synthetic_check()
    _check_f044d35_probe()
...
def main() -> int:
    ...
    if actual != PRIOR_GREEN_D34_BLOB_SHA:
        print(
            "[FAIL] F044-D35 probe-base drift: "
            f"expected={PRIOR_GREEN_D34_BLOB_SHA} actual={actual}",
