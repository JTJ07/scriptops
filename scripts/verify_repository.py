#!/usr/bin/env python3
"""Isolated F044-D38 fifth-following-continuation probe over exact GREEN D37.

No repair is performed. The exact D37 entrypoint is retained byte-for-byte as
`scripts/verify_repository_probe_base.py`; this wrapper adds exactly one
representative where the fifth following sibling owns one ordinary continuation
...
PRIOR_GREEN_D37_BLOB_SHA = "e844cfe5b0c9abef3e147af8efe8075c02cb590e"

core = prior.core
_prior_synthetic_check = core.check_synthetic_rejections_and_transition_positives


def _check_f044d38_probe() -> None:
    source = (
        "- Parent:\n"
    ...
    try:
        ...
        if "publishes forbidden self-promotion" not in str(exc):
            raise
        print(
            "[PASS] F044-D38 probe reproduces fifth-following-continuation false positive"
        )
        return
    raise core.VerificationError(
        "F044-D38 probe NOT REPRODUCED: fifth-following-continuation representative is already accepted"
    )


def _synthetic_check_with_probe() -> None:
    _prior_synthetic_check()
    _check_f044d38_probe()
...
def main() -> int:
    ...
    if actual != PRIOR_GREEN_D37_BLOB_SHA:
        print(
            "[FAIL] F044-D38 probe-base drift: "
            f"expected={PRIOR_GREEN_D37_BLOB_SHA} actual={actual}",
