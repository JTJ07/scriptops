#!/usr/bin/env python3
"""Isolated F044-D36 fourth-following-continuation probe over exact GREEN D35.

No repair is performed. The exact D35 entrypoint is retained byte-for-byte as
`scripts/verify_repository_probe_base.py`; this wrapper adds exactly one
representative where the fourth following sibling owns one ordinary continuation
...
PRIOR_GREEN_D35_BLOB_SHA = "f4c9824b20d95275dfd832b289155d60052b8645"

core = prior.core
_prior_synthetic_check = core.check_synthetic_rejections_and_transition_positives


def _check_f044d36_probe() -> None:
    source = (
        "- Parent:\n"
    ...
    try:
        ...
        if "publishes forbidden self-promotion" not in str(exc):
            raise
        print(
            "[PASS] F044-D36 probe reproduces fourth-following-continuation false positive"
        )
        return
    raise core.VerificationError(
        "F044-D36 probe NOT REPRODUCED: fourth-following-continuation representative is already accepted"
    )


def _synthetic_check_with_probe() -> None:
    _prior_synthetic_check()
    _check_f044d36_probe()
...
def main() -> int:
    ...
    if actual != PRIOR_GREEN_D35_BLOB_SHA:
        print(
            "[FAIL] F044-D36 probe-base drift: "
            f"expected={PRIOR_GREEN_D35_BLOB_SHA} actual={actual}",
