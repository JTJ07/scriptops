...
byte-for-byte at `scripts/verify_repository_f044_lazy_quote_atx.py` and pinned
by Git blob SHA.
...
comment and then another explicit quoted paragraph. The lazy continuation
remains in the first paragraph; the complete HTML comment starts a distinct
leaf; the following quoted paragraph starts another distinct leaf.

The repair is not a generic lazy-paragraph or HTML-block rewrite and does not
generalize to other HTML block types, incomplete comments, fences, thematic
breaks, list-owned quotes, arbitrary block transitions, interaction expansion,
or all block-quote lifecycle handling.
"""
from __future__ import annotations

from pathlib import Path
import verify_repository_f044_lazy_quote_atx as prior

PRIOR_LAZY_QUOTE_ATX_BLOB_SHA = "9699fbe49d570f5b2e6ff0a9ec1ee569d93a2704"
...
def _split_lazy_quote_before_explicit_html_comment_boundaries(text: str) -> str:
    """Flush only a lazy-continued top-level quoted paragraph before quoted HTML."""
...
def _assert_three_leaf_nonclaim(
    label: str,
    units: list[str],
    *,
    middle_fragment: str,
    require_lazy_first: bool = False,
) -> None:
    if len(units) != 3:
        raise core.VerificationError(
            f"{label} must yield exactly three leaves, got {len(units)}"
        )
    normalized = [unit.upper() for unit in units]
    if "THIS FILE" not in normalized[0]:
        raise core.VerificationError(f"{label} first leaf lost quoted paragraph")
    if require_lazy_first and "LAZY CONTINUATION" not in normalized[0]:
        raise core.VerificationError(f"{label} first leaf lost legal lazy continuation")
    if middle_fragment.upper() not in normalized[1]:
        raise core.VerificationError(f"{label} middle leaf mismatch")
    if "GRANTS RELEASE AUTHORITY" not in normalized[2]:
        raise core.VerificationError(f"{label} final quoted paragraph mismatch")
    if any(
        "THIS FILE" in unit and "GRANTS RELEASE AUTHORITY" in unit
        for unit in normalized
    ):
        raise core.VerificationError(f"{label} still fuses separated security fragments")
    if any(core.layer_b_self_promotion_claim(unit) for unit in units):
        raise core.VerificationError(f"{label} still produces false self-promotion claim")
...
def _check_fresh_final_lazy_quote_to_html_comment_regression() -> None:
    ...
    if _split_lazy_quote_before_explicit_html_comment_boundaries(direct_html) != direct_html:
        raise core.VerificationError(
            "fresh-final lazy->HTML repair modified direct HTML-comment control"
        )
    _assert_three_leaf_nonclaim(
        "direct HTML-comment control",
        _prior_authority_soft_wrapped_units(direct_html),
        middle_fragment="<!-- neutral comment -->",
    )

    lazy_atx = _lazy_atx_control()
    if _split_lazy_quote_before_explicit_html_comment_boundaries(lazy_atx) != lazy_atx:
        raise core.VerificationError(
            "fresh-final lazy->HTML repair modified repaired lazy->ATX control"
        )
    _assert_three_leaf_nonclaim(
        "repaired lazy->ATX control",
        _prior_authority_soft_wrapped_units(lazy_atx),
        middle_fragment="# neutral heading",
        require_lazy_first=True,
    )

    direct_atx = _direct_atx_control()
    if _split_lazy_quote_before_explicit_html_comment_boundaries(direct_atx) != direct_atx:
        raise core.VerificationError(
            "fresh-final lazy->HTML repair modified direct ATX control"
        )
    _assert_three_leaf_nonclaim(
        "direct ATX control",
        _prior_authority_soft_wrapped_units(direct_atx),
        middle_fragment="# neutral heading",
    )
    ...
    if _split_lazy_quote_before_explicit_html_comment_boundaries(lazy_control) != lazy_control:
        raise core.VerificationError(
            "fresh-final lazy->HTML repair split ordinary lazy continuation without boundary"
        )
    ...
    _assert_three_leaf_nonclaim(
        "fresh-final lazy->HTML reproduction",
        _prior_authority_soft_wrapped_units(transformed),
        middle_fragment="<!-- neutral comment -->",
        require_lazy_first=True,
    )
    ...
    two_lazy_lines = (
        ...
        "lazy continuation two\n"
        "> <!-- neutral comment -->\n"
        "> grants release authority.\n"
    )
    two_lazy_transformed = _split_lazy_quote_before_explicit_html_comment_boundaries(
        two_lazy_lines
    )
    two_lazy_units = _prior_authority_soft_wrapped_units(two_lazy_transformed)
    if len(two_lazy_units) != 3:
        raise core.VerificationError(
            "fresh-final lazy->HTML repair is accidentally single-lazy-line specific"
        )
    first_upper = two_lazy_units[0].upper()
    if not (
        "THIS FILE" in first_upper
        and "LAZY CONTINUATION ONE" in first_upper
        and "LAZY CONTINUATION TWO" in first_upper
    ):
        raise core.VerificationError(
            "fresh-final lazy->HTML multi-lazy control lost first paragraph ownership"
        )
    ...
    print("[PASS] fresh-final lazy quoted paragraph -> complete quoted HTML reproduction repaired")
    print("[PASS] fresh-final lazy->HTML repair preserves first paragraph lazy continuation")
    print("[PASS] fresh-final lazy->HTML repair preserves direct HTML and lazy/direct ATX controls")
    print("[PASS] fresh-final lazy->HTML repair leaves ordinary lazy continuation unsplit")
    print("[PASS] fresh-final lazy->HTML repair is structural, run-length independent and bounded")
...
def main() -> int:
    ...
    if actual != PRIOR_LAZY_QUOTE_ATX_BLOB_SHA:
        print(
            "[FAIL] prior lazy-quote -> ATX verifier drift: "
            f"expected={PRIOR_LAZY_QUOTE_ATX_BLOB_SHA} actual={actual}",
